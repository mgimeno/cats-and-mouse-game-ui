...
import { type IChessBox } from '../interfaces/chess-box.interface';
  ...
  public static buildChessBoard = (numberOfRows: number, numberOfColumns: number): IChessBox[][] => {
    ...
    for (let rowIndex = 0; rowIndex < numberOfRows; rowIndex++) {
      for (let columnIndex = 0; columnIndex < numberOfColumns; columnIndex++) {
        if (columnIndex !== 0) {
          currentChessBoxColorId =
            currentChessBoxColorId === ChessBoxColorEnum.White ? ChessBoxColorEnum.Black : ChessBoxColorEnum.White;
        }
